...
async function streamHandler(req, res) {
  ...
  try {
    ...
    for (let attempts = 0; attempts < 25; attempts++) {
      ...
      console.log(`[REAL-DEBRID DEBUG] Tentativa ${attempts + 1} para torrent ${torrentId}:`);
      ...
      if (torrentInfo.files) {
        console.log("  Ficheiros do torrent (para seleção):", JSON.stringify(torrentInfo.files.map(f => ({ id: f.id, path: f.path, selected: f.selected, bytes: f.bytes })), null, 2));
      }
      if (torrentInfo.links) {
        console.log("  Links gerados pelo Real-Debrid:", JSON.stringify(torrentInfo.links, null, 2));
      }
      ...
      if (
        ...
        console.log(`[REAL-DEBRID DEBUG] Torrent ${torrentId} descarregado e links disponíveis.`);
        ...
        for (const f of allFiles) {
          if (VIDEO_EXTENSIONS.some((ext) => f.path.toLowerCase().endsWith(ext))) {
            ...
            } else if (bestVideoToSelect === null && isSample && currentSize > largestSizeForSelection) {
        ...
        if (bestVideoToSelect) {
          ...
          console.log(`[REAL-DEBRID DEBUG] Selecionando o melhor ficheiro de vídeo: ${bestVideoToSelect.path} (ID: ${bestVideoToSelect.id})`);
          await axios.post(
          ...
          console.log(`[REAL-DEBRID DEBUG] Melhor ficheiro selecionado para torrent ${torrentId}.`);
        } else {
          // Se não encontrou nenhum ficheiro de vídeo válido após a filtragem
          console.log(`[REAL-DEBRID DEBUG] NENHUM FICHEIRO DE VÍDEO VÁLIDO ENCONTRADO para torrent ${torrentId}.`);
          await axios.delete(
          ...
          return res.status(404).json({
            success: false,
            message: "Nenhum ficheiro de vídeo encontrado no torrent ou Real-Debrid não conseguiu processá-lo.",
          });
        }
      } else if (
        ["magnet_error", "error", "dead"].includes(torrentInfo.status)
      ) {
        console.log(`[REAL-DEBRID DEBUG] Torrent ${torrentId} em status de erro: ${torrentInfo.status}`);
    ...
    if (!torrentInfo.links || torrentInfo.links.length === 0) {
      console.log(`[REAL-DEBRID DEBUG] Torrent ${torrentId} não tem links após o polling (final).`);
      return res.status(404).json({
        success: false,
        message: "Nenhum link disponível no Real-Debrid após a seleção do ficheiro.",
      });
    }
    ...
    console.log(`[REAL-DEBRID DEBUG] Desbloqueando o link: ${linkToUnrestrict}`);
    ...
    console.log(`[REAL-DEBRID DEBUG] URL de stream final obtido: ${finalStreamUrl}`);
